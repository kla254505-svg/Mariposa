...
def _order_date_th(order):
    """orders.py เก็บ opened_at แค่ 'HH:MM' (ไม่มีวันที่) — ไม่พอเช็คว่า order นี้ปิดวันนี้จริงไหม
    ถ้ามี created_at_iso (pending order ของแผน Set & Forget มี field นี้อยู่แล้ว) ใช้อันนั้นแปลงเป็น
    วันที่ไทยแทน ถ้าไม่มีเลย (running order ของ Plan 1-4 เดิมไม่มี field นี้) คืน None — ผู้เรียกจะถือว่า
    'ไม่รู้วันที่แน่ชัด' แล้วนับรวมไปก่อน (กันเหนียวไว้ก่อน ดีกว่าปล่อยผ่านทั้งที่จริงเสียเยอะ)"""
    iso = order.get("created_at_iso")
    if not iso:
        return None
    try:
...
def get_daily_stats(bucket, symbol):
    ...
    for o in orders:
        if o.get("status") != "loss":
            continue
        order_date = _order_date_th(o)
        if order_date is not None and order_date != today:
            continue  # รู้วันที่แน่ชัดว่าไม่ใช่วันนี้ -> ข้าม
