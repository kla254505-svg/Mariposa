...
CONFIG = {
    ...
    # *** ใหม่ (7 ก.ย. 2026): swing_lookback แยกเฉพาะของ 4H Bias (bias_4h.py) ***
    # เดิม analyze_4h_bias() ใช้ "swing_lookback" ตัวเดียวกับที่ Strategy ทุกแผนใช้ร่วมกัน (=7) ซึ่งแปลว่า
    # ต้องรอถึง ~28 ชม. (7 แท่ง x 4H) หลังจุดกลับตัวจริงเกิดขึ้น กว่าจะถูกยืนยันเป็นสวิงและอัปเดตเทรนด์ 4H
    # ทำให้ "4H Trend" ที่ AI Second Opinion เห็น (htf_bias) ค้างสวนทางกับ 1H/15M ได้นานเกินจริง ส่งผลให้
    # AI ประเมิน confidence ได้แค่โซนกลางๆ (WEAK) ต่อเนื่องแม้ตลาดจะเริ่มเปลี่ยนทิศไปแล้วจริงๆ
    # ตั้งค่านี้แยกต่างหาก (ไม่แก้ "swing_lookback" ตัวเดิม) เพื่อให้ "เฉพาะ" การหาเทรนด์/โซน 4H ไวขึ้น
    # โดยไม่กระทบ Strategy แผนอื่นที่ยังอ้างอิง "swing_lookback"=7 เดิมทุกจุด (liquidity pools ของ 4H
    # เองก็ยังใช้ค่าเดิม 7 อยู่ — เปลี่ยนเฉพาะเทรนด์/โซนเท่านั้น ดู bias_4h.py: analyze_4h_bias())
    # เลือก 3 (จากเดิม 7) เป็นจุดกึ่งกลาง: ลดเวลารอยืนยันจาก ~28 ชม. เหลือ ~12 ชม. แต่ยังกรอง noise ได้
    # ระดับหนึ่ง (ไม่ใช่ 1 ที่จะไวเกินจนแกว่งเป็นสัญญาณหลอกบ่อย) ปรับตัวเลขนี้ได้ตามผลที่สังเกตจริงภายหลัง
    "bias4h_swing_lookback": 3,
