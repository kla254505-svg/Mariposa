...
def is_in_news_blackout(kvdb_bucket, symbol, window_minutes=60):
    """
    เช็คว่าตอนนี้อยู่ในช่วง 'ห้ามเทรด' รอบข่าวสำคัญไหม (window_minutes นาที ก่อน-หลังข่าวแต่ละตัว)
    ใช้ cache เดียวกับ pre-news warning/post-news result (ไม่ยิง API เพิ่ม)
    คืนค่า (True, event_dict) ถ้าอยู่ในช่วงห้าม, หรือ (False, None) ถ้าไม่มีข่าวใกล้ๆ
    """
    raw = kv_get(kvdb_bucket, f"calendar_events_{symbol}")
    if not raw:
        return False, None
